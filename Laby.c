...
#define LARGEUR3 84 // definition des valeurs de la taille de nos labyrinthes
...
int is_finish();// d�fini quand est ce que la labyrinth est fini
...
char yourLabyrinth[HAUTEUR1][LARGEUR1]; // labyrinthe global, utilis� pour la cr�ation
int yoursSet=0;// pour savoir si on a d�j� cr�� un labyrinthe, c'est plus simple d'en faire une globale
void movement();
...
int menu(){
    int menuChoice;
    clearTerminal();
    printf("Bonjour bienvenue sur EL LABYRENTO DE LA MUERTE !!!\n\nMENU >>>\n\n"); //notre th�me est "l'espagnol mais en mauvais"
// donc une partie du text est en espagnol mal �crit
    printf("%s", menutxt);
...
void levelChoice(){
    ...
    printf("\nLe menu : \n");
    printf("1 -> Niveau FACILE = Esto MUI FACIL ");
    if(finish1==1){printf("           niveau deja termine");}
    printf("\n2 -> Niveau MOYEN = te respeto mucho !!! ");
    if(finish2==1){printf("      niveau deja termine");}
    printf("\n3 -> Aie Aie Aie CARAMBA Buena suerte, chico !!");
    if(finish3==1){printf("niveau deja termine");}
    printf("\n4 -> puedes juegar a tu labyrente ");if(yoursSet==0){printf("pero no puedes porque ne lo ha creado");}
    if(finishyours==1){printf("           niveau deja termine");}
    printf("\n5 -> revenir en arriere");
    if(finishyours==1&&finish3==1&&finish2==1&&finish1==1){
        printf("\n6 -> nouveau niveau secret !");
    }
    printf("\n>>>");
    ...
    switch(difficulty) {
        ...
        case 4 :
            if(yoursSet==1){
                ...
                printf("veuillez d'abord creer un labyrinthe");// si l'utilisateur n'a pas cr�er de labyrinth alors il est renvoy� ici
                levelChoice();
            }
            break;
        case 5 :
...
// v�rifie si le movement de l'user est valide
...
void play3() {
    ...
    //d�finis le labyrinthe
    char grid[HAUTEUR3][LARGEUR3] = {
"# ##################################################################################",
"#   #    ######       ########     # ## # #############   #####       ## ##  #######",
"## ## ## # ######## #    #     ### ####      ###          ###  ## ### # # # # # ####",
"##    ##  ##      # ####   ###     #####  ###### ######       #####   # ##        ##",
...
"####################################################.###############################"};
    ...
    movement(HAUTEUR3,LARGEUR3,grid);

    chronoFin = time(NULL);

    clearTerminal();

    printf("BRAVOOOOO !!!!! Vous avez r�ussi ce niveau \n");
...
"##############################.##################"};
    ...
    movement(HAUTEUR1,LARGEUR1,grid);
...
"########.###################################"};
    ...
    movement(HAUTEUR2,LARGEUR2,grid);
...
"#############################################.###"};
    ...
    clearTerminal();
    printf("Bravo !!! Vous avez termin� tous les labyrinthes pr�construits !\nSi nous arrivons � le faire, n'h�sitez pas � jouer aux labyrinthes al�atoirs !\n");
    printf("Vous avez mis  %ld", chronoFin - chronoDebut);
    printf(" secondes a finir le Labyrinthe \n ");
    getch();
    finish1=1;
...
int is_validcreation(int x, int y, int hauteur, int largeur, char grid[hauteur][largeur]) {
    if(y>=hauteur-1&&grid[y][x]=='#'||x>=largeur-2||x<=0||y<=0){ //les limites du tableau et je veux quand
        return 0;                                               //meme pouvoir aller au '.' donc je fais un check pour'#'
    }
    return 1;
}

int is_finishcreation(int x, int y, int hauteur, int largeur, char grid[hauteur][largeur]) {
    if (y==hauteur-1){ //
        return 1;
    }
    return 0;
...
"#############################################.###"};
    ...
    do {
        ...
        switch(getch()) {
            ...
            case 'f': if(doFill==0){doFill=1;}else{doFill=0;}
        }
        if(is_validcreation(move_x, move_y, HAUTEUR1, LARGEUR1, grid)) {
            x = move_x; y = move_y;
            if(grid[y][x]!='.'){
                if(doFill){
                    grid[y][x] = '#';
    ...
    } while(! is_finishcreation(x, y, HAUTEUR1, LARGEUR1, grid));
    refresh();
    clrtoeol();
    refresh();
    endwin();
...
// void freegrid(char ** grid, int hauteur){
//     int i;
//     for(i=0;i<hauteur;++i){
//         free(grid[i]);
//     }
